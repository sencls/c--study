...
{
    _fun_callback[MSG_HELLO_WORD] = [this](std::shared_ptr<CSession> session, const short &msg_id, const string &msg_data)
    { HelloWordCallBack(session, msg_id, msg_data); };
}

void LogicSystem::HelloWordCallBack(std::shared_ptr<CSession> session, const short &msg_id, const string &msg_data)
{
    Json::Reader reader;
    Json::Value root;
